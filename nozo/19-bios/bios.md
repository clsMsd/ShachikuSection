...
（ファイル上のオフセットは`0x3fff0`だがqemuがこのファイルをメモリにロードするときによしなにしてくれるのだと思う）
...
ちなみにqemuは256KBのバイナリファイルであればBIOSとして読み込むことができるっぽい（正確な仕様は知らない）。
なので下記のような何もしないBIOSをつくることもできる。
...
このプログラムはリセットベクタの位置で自己jmpする命令を置いたもの（それ以外は0で埋める）。

```
$ nasm bios.S -o bios.bin && objdump -b binary -m i8086 -D bios.bin

bios.bin:     file format binary
...
00000000 <.data>:
        ...
   3fff0:       eb fe                   jmp    0x3fff0
```

qemuコマンドに`-bios`オプションでBIOSのバイナリファイルを指定する。

```
# qemu-system-i386 -bios bios.bin -monitor stdio
...
何もしないプログラムなので当然qemuのディスプレイには何も表示されない。
